...
from database import (
    get_db, init_db,
    Order, Downtime, MachineProductHistory, Machine,
    Component, BOM, ComponentSchedule,
    Completion   # ← 新增這個
)
...
from schemas import (
    ...
    OrderDetailResponse,
    CompletionCreate, CompletionResponse   # ← 新增
)
...
def create_completions_batch(
    ...
    for payload in payloads:
        try:
            # 你唯一值是 completion_no，所以重複就跳過
            exists = db.query(Completion).filter(Completion.completion_no == payload.completion_no).first()
            if exists:
                skipped += 1
